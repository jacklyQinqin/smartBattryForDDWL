...
void task6_task(void *pvParameters)
{
	...
	while(1)
    {
        if(time++ >= 100)
        {
            vTaskSuspend(KEYSCANTask_Handler);
            vTaskSuspend(MC20Task_Handler);
            vTaskSuspend(FLOATTask_Handler);
            vTaskSuspend(TASK5Task_Handler);
            vTaskSuspend(TASK7Task_Handler);
        }
        if(time++ >= 200)
        {
            vTaskResume(KEYSCANTask_Handler);
            vTaskResume(MC20Task_Handler);
            vTaskResume(FLOATTask_Handler);
            vTaskResume(TASK5Task_Handler);
            vTaskResume(TASK7Task_Handler);
            time = 0;
        }
        GPIO_SetBits(GPIOE, GPIO_Pin_3);
        vTaskDelay(500);
